...
class HLSStreamWorker(SegmentedStreamWorker):
    ...
    SEGMENT_QUEUE_TIMING_THRESHOLD_FACTOR = 3
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.playlist_target_duration: float = 0
        self.playlist_sequence: int = -1
        self.playlist_sequences: List[Sequence] = []
        self.playlist_sequences_last: datetime = datetime.now()
        self.playlist_reload_last: datetime = datetime.now()
    ...
    def reload_playlist(self):
        ...
        self.playlist_target_duration = playlist.target_duration or 0
    ...
    def _segment_queue_timing_threshold_reached(self) -> bool:
        threshold = self.playlist_target_duration * self.SEGMENT_QUEUE_TIMING_THRESHOLD_FACTOR
        if datetime.now() <= self.playlist_sequences_last + timedelta(seconds=threshold):
            return False
